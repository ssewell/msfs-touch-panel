...
namespace MSFSTouchPanel.ArduinoAgent
{
    public class ArduinoProvider
    {
        ...
        public ArduinoProvider()
        {
            ...
            {
                Logger.ServerLog($"Arduino Error: {ex.Message}", LogLevel.ERROR);
            }
        }
        ...
        public void Start()
        {
            if (!IsConnected)
            {
                try
                {
                    ...
                    Logger.ServerLog("Arduino connected", LogLevel.INFO);
                }
                catch (Exception ex)
                {
                    Logger.ServerLog($"Arduino Connection Error - {ex.Message}", LogLevel.ERROR);
                }
            }
        }
        ...
        public void Stop()
        {
            if (_serialPort.IsOpen)
            {
                ...
                {
                    _serialPort.Close();
                    Logger.ServerLog("Arduino disconnected", LogLevel.INFO);
                }
                catch (Exception ex) 
                {
                    Logger.ServerLog($"Arduino Disconnect Error - {ex.Message}", LogLevel.ERROR);
                }
            }
